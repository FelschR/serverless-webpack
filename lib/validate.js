...
module.exports = {
  validate() {
    ...
    const getEntryForFunction = (name, serverlessFunction) => {
      ...
      const handlerEntry = handlerFile.split("/").pop();

      // Create a valid entry key
      return {
        [handlerEntry]: `./${handlerFile}${ext}`
      };
    };
    ...
    const processConfig = _config => {
      ...
      // update handlers in serverless confuguration
      const functions = this.serverless.service.functions;
      Object.keys(functions)
        .filter(entry => helpers.hasNodeRuntime(entry, this.serverless))
        .forEach(entry => {
          this.serverless.cli.log("Updating handler: " + entry);
          try {
            functions[entry].handler = functions[entry].handler.split('/').pop();
          } catch (error) {
